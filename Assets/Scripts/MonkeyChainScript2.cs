...
public class MonkeyChainScript2 : MonoBehaviour
{
    float angle = Mathf.Atan2(9, 16) * Mathf.Rad2Deg * -1f;
    float speed = 25f;
    ...
    void FixedUpdate()
    {
        if (transform.position.x < 0f) transform.position += new Vector3(Mathf.Cos(angle), Mathf.Sin(angle), 0f) * Time.fixedDeltaTime * speed;
